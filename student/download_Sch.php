...
$st_id = $_GET['st_id'];
$sr = 1;
$res=mysqli_query($con,"SELECT * FROM `schedule` inner join faculty on schedule.f_id = faculty.f_id inner join room on schedule.r_id = room.r_id inner join course on schedule.c_id = course.c_id inner join select_course on select_course.c_id = course.c_id where select_course.st_id = '$st_id'");
